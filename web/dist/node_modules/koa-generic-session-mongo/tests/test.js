...
describeStore('store from url', {url: 'mongodb://website:zombie.123@127.0.0.1:27017/test'}, {cleanDb: true});

describe('test auth', function() {
  let db;

  before(function *() {
      db = yield thunkify(MongoClient.connect)('mongodb://website:zombie.123@127.0.0.1:27017/testauth');
  });
...
describe('closed db', function() {
  ...
  before(function *() {
    db = yield thunkify(MongoClient.connect)('mongodb://website:zombie.123@127.0.0.1:27017/test');
...
describe('url info exclusive', function () {
  it('should fail if host and url provided', function *() {
    assert.throw(() => {new MongoStore({url: 'mongodb://website:zombie.123@127.0.0.1:27017', host: 'localhost'})});
  });
  it('should fail if port and url provided', function *() {
    assert.throw(() => {new MongoStore({url: 'mongodb://website:zombie.123@127.0.0.1:27017', port: '27017'})});
  });
  it('should fail if db and url provided', function *() {
    assert.throw(() => {new MongoStore({url: 'mongodb://website:zombie.123@127.0.0.1:27017', db: 'admin'})});
  });
  it('should fail if ssl and url provided', function *() {
    assert.throw(() => {new MongoStore({url: 'mongodb://website:zombie.123@127.0.0.1:27017', ssl: true})});
  });
});
